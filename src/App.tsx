  ...
  return (
    <div className="img-message">
      ...
      <div className="img-actions">
        ...
        <button className="img-action-btn" onClick={onRegenerate} title="Generate new variation" aria-label="Generate new image variation">
        ...
        <div className={`input-box pill-box ${listening ? "listening" : ""} ${disabled ? "streaming-active" : ""} ${isImageMode ? "image-mode" : ""} ${stagedFiles.length > 0 ? "has-attachments" : ""}`}>
          ...
          <div className="input-right-actions">
            ...
            <button type="button" className={`input-icon-btn ${listening ? "active-mic" : ""}`}
              title="Voice input" aria-label="Toggle voice input" onClick={toggleVoice}>
              {listening ? <MicOff size={16} /> : <Mic size={16} />}
            </button>

            {disabled ? (
              <motion.button onClick={onStop} className="send-btn stop-btn" title="Stop generation" aria-label="Stop generation"
        ...
        <motion.div className="modal-overlay" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onClick={onClose}>
          <motion.div className="modal wide-modal" initial={{ opacity: 0, scale: 0.95, y: 12 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95 }} transition={{ duration: 0.18, ease: [0.16, 1, 0.3, 1] }} onClick={e => e.stopPropagation()}>
            <div className="modal-header">
              ...
              <button onClick={onClose} className="modal-close" title="Close" aria-label="Close modal"><X size={15} /></button>
            </div>
            {/* Tabs */}
      ...
      <div className={`sidebar ${open ? "open" : ""}`}>
        <div className="sidebar-header">
          <div className="sidebar-brand-row">
            <button className="brand-dropdown-btn" onClick={onSettings} title="Settings" aria-label="Open settings">
            ...
            <div className="main">
              ...
              <div className="header">
                <div className="header-left">
                  <button className="mobile-menu-btn" onClick={() => setMobileSidebar(true)} title="Open menu" aria-label="Open mobile menu"><Menu size={18} /></button>
                  {!desktopSidebar && (
                    <div className="tooltip-wrap">
