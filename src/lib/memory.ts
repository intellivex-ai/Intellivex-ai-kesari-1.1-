...
function openDB(): Promise<IDBDatabase> {
  if (!dbPromise) {
    dbPromise = new Promise((resolve, reject) => {
      ...
      req.onerror = () => {
        dbPromise = null; // Reset on error to allow retries
        reject(req.error)
      }
    })
  }
...
export async function addMemory(
  ...
  for (const chunk of chunks) {
    // Yield to the event loop before processing each chunk to prevent UI jank
    // during heavy CPU-bound tokenization of large texts.
  ...
  // Execute chunk persistence concurrently
  ...
  if (all.length > MAX_CHUNKS) {
    ...
    const deletePromises = sorted.slice(0, all.length - MAX_CHUNKS).map(old => deleteChunk(old.id))
