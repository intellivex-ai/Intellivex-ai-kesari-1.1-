...
**Learning:** Found that static markdown components (`TextBlock`, `CodeBlock`, `MarkdownTable`, etc) inside `MessageRow` were re-rendering unnecessarily during token-by-token streaming, causing O(N^2) bottlenecks even with `MessageRow` memoized. `MarkdownTable` specifically needed a custom comparison function because its array props are recreated every render.
**Action:** Wrapped inner static rendering components with `React.memo()` to prevent cascading re-renders during active message generation.
