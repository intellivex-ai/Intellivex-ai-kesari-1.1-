...
## 2026-05-18 - Binding dynamic states to A11y Attributes
**Learning:** Icon-only buttons with dynamic states (e.g. Stop reading vs Read aloud) will incorrectly convey intent to screen readers if the `aria-label` attribute doesn't change dynamically.
**Action:** When creating a button with a dynamic `title` state, make sure the `aria-label` also maps to the same conditional state.
